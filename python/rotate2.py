...
def localMinima(data, threshold):
    ...
    if threshold is not None:
        #peaks = data < threshold :::: TODO
        peaks = data > threshold
    else:
        peaks = ones(data.shape, dtype=data.dtype)

    # peaks &= data == minimum_filter(data, size=(3,) * data.ndim) :::: TODO
...
def blobLOG(data, scales=range(1, 10, 1), threshold=-30):
    ...
    log = empty((len(scales),) + data.shape, dtype=data.dtype)  #TODO: float 16 or float 32
    print("log dtype", log.dtype)
    for slog, scale in zip(log, scales):
        slog[...] = scale ** 2 * gaussian_laplace(data, scale)

    print("log min max ",np.min(log), np.max(log))
